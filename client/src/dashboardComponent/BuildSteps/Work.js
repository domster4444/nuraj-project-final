...
const Work = () => {
  ...
  const handleChange = (e, id) => {
    ...
    const updatedWorkList = workList.map((work) => (work.id === id ? Object.assign(work, { id: uuidv4(), [name]: value }) : work));
  ...
  return (
    <>
        ...
        {workList.map((work, index) => (
          <AccordionItem key={index}>
            ...
            <AccordionPanel pb={4}>
              ...
              <Button rightIcon={<MdDelete />} onClick={() => deleteWork(work.id)} mt={3} colorScheme={"red"}>
                Delete
              </Button>
            </AccordionPanel>
          </AccordionItem>
      ...
      {workList.length < 3 && (
        <Button style={{ background: "black", color: "white" }} my={5} onClick={addMore}>
          Add More
        </Button>
      )}
    </>
