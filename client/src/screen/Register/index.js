...
const Index = () => {
  ...
  const [verifiedRegisterUser, { isLoading }] = useVerifiedRegisterUserMutation();
    ...
    <main>
      <RegisterContainer>
        ...
        <FormBox onSubmit={formik.handleSubmit}>
          <Text>Register</Text>
          <label htmlFor='name'>
            <Input {...formik.getFieldProps("name")} elementSize='small' placeholder='Enter your name' name='name' type='name' />

            {formik.errors.name && formik.touched.name ? <Alert type='error'>{formik.errors.name}</Alert> : null}
          </label>
          <label htmlFor='email'>
