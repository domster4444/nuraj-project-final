...
exports.createToken = (dataToStoreInToken, userid, secret, expireDate = "15d") => {
  try {
    ...
    const token = jwt.sign({ data: dataToStoreInToken }, uniqueSecretKeyForEachUser, {
      expiresIn: expireDate,
    });
    console.log(token);
    return token;
...
exports.verifyToken = (tokenToBeVerified, secretKeyUsedForCreatingToken) => {
  try {
    const dataStoredInsideJWT = jwt.verify(tokenToBeVerified, secretKeyUsedForCreatingToken);
